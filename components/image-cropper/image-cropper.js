Component({
  ...
  attached: function() {
    this.data.info = wx.getSystemInfoSync(), this._watcher(), this.data.INIT_IMGWIDTH = this.data.img_width, this.data.INIT_IMGHEIGHT = this.data.img_height, this.setData({
      _canvas_height: this.data.height,
      _canvas_width: this.data.width
    }), this._initCanvas(), this.data.imgSrc && (this.data.imgSrc = this.data.imgSrc), this._initImageSize(), this._computeCutSize(), this._cutDetectionPosition(), this._canvasDetectionPosition(), this.triggerEvent("load", {
      cropper: this
    })
  },
  methods: {
    ...
    setTransform: function(t) {
      if (t) {
        ...
        t.scale && (a = (a = (a = this.data.scale + t.scale) <= this.data.min_scale ? this.data.min_scale : a) >= this.data.max_scale ? this.data.max_scale : a), this.data.scale = a;
        var i = this.data.cut_left,
          h = this.data.cut_top;
        t.cutX && (this.setData({
          cut_left: i + t.cutX
    ...
    _imgMarginDetectionPosition: function(t) {
      if (this.data.limit_move) {
        var a = this.data._img_left,
          i = this.data._img_top,
          h = (t = t || this.data.scale, this.data.img_width),
          e = this.data.img_height;
        this.data.angle / 90 % 2 && (h = this.data.img_height, e = this.data.img_width), a = this.data.cut_left + h * t / 2 >= a ? a : this.data.cut_left + h * t / 2, a = this.data.cut_left + this.data.width - h * t / 2 <= a ? a : this.data.cut_left + this.data.width - h * t / 2, i = this.data.cut_top + e * t / 2 >= i ? i : this.data.cut_top + e * t / 2, i = this.data.cut_top + this.data.height - e * t / 2 <= i ? i : this.data.cut_top + this.data.height - e * t / 2, this.setData({
          _img_left: a,
          _img_top: i,
          scale: t
        })
    ...
    _imgMarginDetectionScale: function() {
      if (this.data.limit_move) {
        var t = this.data.scale,
          a = this.data.img_width,
          i = this.data.img_height;
        this.data.angle / 90 % 2 && (a = this.data.img_height, i = this.data.img_width), a * t < this.data.width && (t = this.data.width / a), i * t < this.data.height && (t = Math.max(t, this.data.height / i)), this._imgMarginDetectionPosition(t)
      }
    },
    _setData: function(t) {
      var a = {};
    ...
    _start: function(t) {
      if (this.data._flag_img_endtouch = !1, 1 == t.touches.length) this.data._touch_img_relative[0] = {
    ...
    _move: function(t) {
      if (!this.data._flag_img_endtouch && this.data.MOVE_THROTTLE_FLAG) {
        if (this.data.MOVE_THROTTLE_FLAG = !1, this._move_throttle(), this._moveDuring(), 1 == t.touches.length) {
          var a = t.touches[0].clientX - this.data._touch_img_relative[0].x,
            i = t.touches[0].clientY - this.data._touch_img_relative[0].y;
          this.data._img_left = a, this.data._img_top = i, this._imgMarginDetectionPosition(), this.setData({
            _img_left: this.data._img_left,
            _img_top: this.data._img_top
          })
        } else {
          var h = Math.abs(t.touches[0].clientX - t.touches[1].clientX),
            e = Math.abs(t.touches[0].clientY - t.touches[1].clientY),
            s = Math.sqrt(Math.pow(h, 2) + Math.pow(e, 2)),
            d = this.data.scale * (s / this.data._hypotenuse_length),
            _ = 0;
          d = (d = d <= this.data.min_scale ? this.data.min_scale : d) >= this.data.max_scale ? this.data.max_scale : d, this.data.scale = d, this._imgMarginDetectionScale();
          var n = [{
            x: t.touches[0].clientX - this.data._img_left,
            y: t.touches[0].clientY - this.data._img_top
          }, {
          ...
          if (!this.data.disable_rotate) {
            var c = 180 / Math.PI * Math.atan2(n[0].y, n[0].x) - 180 / Math.PI * Math.atan2(this.data._touch_img_relative[0].y, this.data._touch_img_relative[0].x),
              o = 180 / Math.PI * Math.atan2(n[1].y, n[1].x) - 180 / Math.PI * Math.atan2(this.data._touch_img_relative[1].y, this.data._touch_img_relative[1].x);
            0 != c ? _ = c : 0 != o && (_ = o)
          }
          this.data._touch_img_relative = n, this.data._hypotenuse_length = Math.sqrt(Math.pow(h, 2) + Math.pow(e, 2)), this.setData({
    ...
    _end: function(t) {
      this.data._flag_img_endtouch = !0, this._moveStop()
    },
    _click: function(t) {
      var a = this;
      this.data.imgSrc ? this._draw((function() {
        var i = t.detail ? t.detail.x : t.touches[0].clientX,
          h = t.detail ? t.detail.y : t.touches[0].clientY;
        i >= a.data.cut_left && i <= a.data.cut_left + a.data.width && h >= a.data.cut_top && h <= a.data.cut_top + a.data.height && wx.canvasToTempFilePath({
          width: a.data.width * a.data.export_scale,
          height: Math.round(a.data.height * a.data.export_scale),
          destWidth: a.data.width * a.data.export_scale,
          destHeight: Math.round(a.data.height) * a.data.export_scale,
    ...
    _draw: function(t) {
      ...
      if (this.data.imgSrc) {
        var i = function() {
          var i = a.data.img_width * a.data.scale * a.data.export_scale,
            h = a.data.img_height * a.data.scale * a.data.export_scale,
            e = a.data._img_left - a.data.cut_left,
            s = a.data._img_top - a.data.cut_top;
          a.data.ctx.translate(e * a.data.export_scale, s * a.data.export_scale), a.data.ctx.rotate(a.data.angle * Math.PI / 180), a.data.ctx.drawImage(a.data.imgSrc, -i / 2, -h / 2, i, h), a.data.ctx.draw(!1, (function() {
            t && t()
          }))
        };
    ...
    _cutTouchMove: function(t) {
      ...
      if (this.data._flag_cut_touch && this.data.MOVE_THROTTLE_FLAG) {
        ...
        var i = this.data.width,
          ...
          d = function() {
            i = i <= a.data.max_width ? i >= a.data.min_width ? i : a.data.min_width : a.data.max_width, h = h <= a.data.max_height ? h >= a.data.min_height ? h : a.data.min_height : a.data.max_height
          },
          _ = function() {
            return (i > a.data.max_width || i < a.data.min_width || h > a.data.max_height || h < a.data.min_height) && a.data.disable_ratio ? (d(), !1) : (d(), !0)
          };
        switch (h = this.data.CUT_START.height + (this.data.CUT_START.corner > 1 && this.data.CUT_START.corner < 4 ? 1 : -1) * (this.data.CUT_START.y - t.touches[0].clientY), this.data.CUT_START.corner) {
          case 1:
            if (i = this.data.CUT_START.width + this.data.CUT_START.x - t.touches[0].clientX, this.data.disable_ratio && (h = i / (this.data.width / this.data.height)), !_()) return;
    ...
    _cutTouchStart: function(t) {
      var a = t.touches[0].clientX,
        ...
        h = this.data.cut_top + this.data.height - 30,
        e = this.data.cut_top + this.data.height + 20,
        s = this.data.cut_left + this.data.width - 30,
        d = this.data.cut_left + this.data.width + 30,
        _ = this.data.cut_top - 30,
        n = this.data.cut_top + 30,
        c = this.data.cut_left + this.data.width - 30,
        o = this.data.cut_left + this.data.width + 30,
        u = this.data.cut_top - 30,
        l = this.data.cut_top + 30,
        g = this.data.cut_left - 30,
        m = this.data.cut_left + 30,
        r = this.data.cut_top + this.data.height - 30,
        f = this.data.cut_top + this.data.height + 30,
        w = this.data.cut_left - 30,
        T = this.data.cut_left + 30;
      a > s && a < d && i > h && i < e ? (this._moveDuring(), this.data._flag_cut_touch = !0, this.data._flag_img_endtouch = !0, this.data.CUT_START = {
        width: this.data.width,
        height: this.data.height,
        x: a,
        y: i,
        corner: 4
      }) : a > c && a < o && i > _ && i < n ? (this._moveDuring(), this.data._flag_cut_touch = !0, this.data._flag_img_endtouch = !0, this.data.CUT_START = {
        width: this.data.width,
        height: this.data.height,
        x: a,
        y: i,
        ...
        corner: 3
      }) : a > g && a < m && i > u && i < l ? (this._moveDuring(), this.data._flag_cut_touch = !0, this.data._flag_img_endtouch = !0, this.data.CUT_START = {
        width: this.data.width,
        height: this.data.height,
        cut_top: this.data.cut_top,
        cut_left: this.data.cut_left,
        ...
        y: i,
        corner: 2
      }) : a > w && a < T && i > r && i < f && (this._moveDuring(), this.data._flag_cut_touch = !0, this.data._flag_img_endtouch = !0, this.data.CUT_START = {
        width: this.data.width,
        height: this.data.height,
        cut_top: this.data.cut_top,
        cut_left: this.data.cut_left,
